...
from ..utils.utils_visual import display_reconstructed_and_flip_images, display_umap_for_latent_dynamic
import wandb
...
class VAUBGPModule(LightningModule):
    ...
    def __init__(
        ...
        self.train_src_acc = Accuracy(task="multiclass", num_classes=31)
        self.train_tgt_acc = Accuracy(task="multiclass", num_classes=31)
        self.val_src_acc = Accuracy(task="multiclass", num_classes=31)
        self.val_tgt_acc = Accuracy(task="multiclass", num_classes=31)
        self.test_src_acc = Accuracy(task="multiclass", num_classes=31)
        self.test_tgt_acc = Accuracy(task="multiclass", num_classes=31)
    ...
    def training_step(
        ...
        if (((self.global_step % self.hparams.loops) == 0) and
            ...
            score = self.score_model.get_mixing_score_fn(
                ...
                alpha=None) - 0.05 * z.view(-1, C, H, W)
    ...
    def model_step(self, batch) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        outputs_ori = self.classifier(mean1.view((z1.shape[0], -1)).detach())
        outputs_flip = self.classifier(mean2.view((z2.shape[0], -1)).detach())
    ...
    def validation_step(self, batch, batch_idx: int) -> None:
        ...
        if self.global_rank == 0:
            ...
            plt_umap = display_umap_for_latent_dynamic(
            ...
            logger_experiment.log({
                "UMAP for Latent": wandb.Image(fig_total),
                "UMAP for Each Class": wandb.Image(fig_individual),
            })

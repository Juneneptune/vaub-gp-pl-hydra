...
class VAUBGPModule(LightningModule):
    ...
    def __init__(
        ...
        optimizer_cls1: torch.optim.Optimizer,
        optimizer_cls2: torch.optim.Optimizer,
        ...
        self.classifier_race = classifier1(input_dim=latent_dim)
        self.classifier_age = classifier2(input_dim=latent_dim)
        ...
        self.train_src_race_acc = Accuracy(task="multiclass", num_classes=7)
        self.train_tgt_race_acc = Accuracy(task="multiclass", num_classes=7)
        self.train_src_age_acc = Accuracy(task="multiclass", num_classes=9)
        self.train_tgt_age_acc = Accuracy(task="multiclass", num_classes=9)
        self.val_src_race_acc = Accuracy(task="multiclass", num_classes=7)
        self.val_tgt_race_acc = Accuracy(task="multiclass", num_classes=7)
        self.val_src_age_acc = Accuracy(task="multiclass", num_classes=9)
        self.val_tgt_age_acc = Accuracy(task="multiclass", num_classes=9)
        self.test_src_race_acc = Accuracy(task="multiclass", num_classes=7)
        self.test_tgt_race_acc = Accuracy(task="multiclass", num_classes=7)
        self.test_src_age_acc = Accuracy(task="multiclass", num_classes=9)
        self.test_tgt_age_acc = Accuracy(task="multiclass", num_classes=9)
    ...
    def on_train_start(self) -> None:
        ...
        self.val_src_race_acc.reset()
        self.val_tgt_race_acc.reset()
        self.val_src_age_acc.reset()
        self.val_tgt_age_acc.reset()
    ...
    def training_step(
        ...
        if self.is_MoG_prior:
            optimizer_vae_1, optimizer_vae_2, optimizer_score, optimizer_cls1, optimizer_cls2, optimizer_prior = self.optimizers()
        else:
            optimizer_vae_1, optimizer_vae_2, optimizer_score, optimizer_cls1, optimizer_cls2 = self.optimizers()
        ...
        if (((self.global_step % self.hparams.loops) == 0) and
            ...
            score = self.score_model.get_mixing_score_fn(
                z.view(-1, C, H, W).detach(), 1*torch.ones(z.shape[0], device=z.device).type(torch.long),
                latent_noise_idx.type(torch.long), detach=True, is_residual=True,
                is_vanilla=self.hparams.is_vanilla,
                alpha=None) - 0.05 * z.view(-1, C, H, W).detach()
            ...
            # CLS loss
            output_race_cls = self.classifier_race(mean1.view((z1.shape[0], -1)).detach())
            output_race_tgt = self.classifier_race(mean2.view((z2.shape[0], -1)).detach())
            # output_cls = self.classifier(z1)
            classifier_race_loss = F.cross_entropy(output_race_cls, label1[:, 0], reduction='none').mean()
            
            output_age_cls = self.classifier_age(mean1.view((z1.shape[0], -1)).detach())
            output_age_tgt = self.classifier_age(mean2.view((z2.shape[0], -1)).detach())
            # output_cls = self.classifier(z1)
            classifier_age_loss = F.cross_entropy(output_age_cls, label1[:, 2], reduction='none').mean()
            ...
            tot_loss.backward()
            classifier_race_loss.backward()
            classifier_age_loss.backward()
            ...
            self.log("loss/classifier_age_loss", classifier_age_loss, on_step=True, on_epoch=False, sync_dist=True)
            self.log("loss/classifier_race_loss", classifier_race_loss, on_step=True, on_epoch=False, sync_dist=True)
            ...
            self.train_src_race_acc(output_race_cls, label1[:, 0])
            self.train_tgt_race_acc(output_race_tgt, label2[:, 0])
            self.train_src_age_acc(output_age_cls, label1[:, 2])
            self.train_tgt_age_acc(output_age_tgt, label2[:, 2])
            self.log("train/src_race_acc", self.train_src_race_acc, on_step=True, on_epoch=False, sync_dist=True)
            self.log("train/tgt_race_acc", self.train_tgt_race_acc, on_step=True, on_epoch=False, sync_dist=True)
            self.log("train/src_age_acc", self.train_src_age_acc, on_step=True, on_epoch=False, sync_dist=True)
            self.log("train/tgt_age_acc", self.train_tgt_age_acc, on_step=True, on_epoch=False, sync_dist=True)
        ...
        if not self.is_MoG_prior:
            dsm_loss = self.score_model.update_score_fn(z.view((-1, C, H, W)).detach(),
    ...
    # def on_train_epoch_end(self) -> None:
    #     # Compute the mean loss for the entire epoch
    #     train_loss_value = self.train_loss.compute()
    
    #     # Log the mean loss for the entire epoch
    #     self.log("train/loss_epoch", train_loss_value, on_epoch=True, sync)
    ...
    def model_step(self, batch) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        outputs_race_ori = self.classifier_race(mean1.view((z1.shape[0], -1)).detach())
        outputs_race_flip = self.classifier_race(mean2.view((z2.shape[0], -1)).detach())
        
        outputs_age_ori = self.classifier_age(mean1.view((z1.shape[0], -1)).detach())
        outputs_age_flip = self.classifier_age(mean2.view((z2.shape[0], -1)).detach())

        self.val_src_race_acc(outputs_race_ori, label1[:, 0])
        self.val_tgt_race_acc(outputs_race_flip, label2[:, 0])
        
        self.val_src_age_acc(outputs_age_ori, label1[:, 2])
        self.val_tgt_age_acc(outputs_age_flip, label2[:, 2])

        return outputs_race_ori, outputs_race_flip, outputs_age_ori, outputs_age_flip

    def validation_step(self, batch, batch_idx: int) -> None:

        _, (x1, _, label1), (x2, _, label2) = batch
        outputs_race_ori, outputs_race_flip, outputs_age_ori, outputs_age_flip = self.model_step(batch)
        ...
        self.val_src_age_acc(outputs_age_ori, label1[:, 2])
        self.val_tgt_age_acc(outputs_age_flip, label2[:, 2])
        self.log("val/src_race_acc", self.val_src_race_acc, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/tgt_race_acc", self.val_tgt_race_acc, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/src_age_acc", self.val_src_age_acc, on_step=False, on_epoch=True, prog_bar=True)
        self.log("val/tgt_age_acc", self.val_tgt_age_acc, on_step=False, on_epoch=True, prog_bar=True)
        ...
        if self.global_rank == 0:
            ...
            plt_umap = display_umap_for_latent(
                ...
                data_1=x1[:300],
                data_2=x2[:300],
                label_1=label1[:300, 0],
                label_2=label2[:300, 0]
            )
            logger_experiment.log({
                "UMAP for Latent Race": wandb.Image(fig_total),
                "UMAP for Each Race": wandb.Image(fig_individual),
            })
            ...
            plt_umap = display_umap_for_latent(
                ...
                data_1=x1[:300],
                data_2=x2[:300],
                label_1=label1[:300, 2],
                label_2=label2[:300, 2]
            )
            logger_experiment.log({
                "UMAP for Latent Age": wandb.Image(fig_total),
                "UMAP for Each Age": wandb.Image(fig_individual),
            })
            fig_individual.tight_layout()
            plt.close(fig_total)
            plt.close(fig_individual)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        self.val_src_race_acc.compute()  # get current val acc
        self.val_tgt_race_acc.compute()  # get current val acc
        self.val_src_age_acc.compute()  # get current val acc
        self.val_tgt_age_acc.compute()  # get current val acc
    ...
    def test_step(self, batch, batch_idx: int) -> None:
        ...
        # update and log metrics
        self.test_src_race_acc(outputs_race_ori, label1[:, 0])
        self.test_tgt_race_acc(outputs_race_flip, label2[:, 0])
        self.test_src_age_acc(outputs_age_ori, label1[:, 2])
        self.test_tgt_age_acc(outputs_age_flip, label2[:, 2])
        self.log("test/src_race_acc", self.test_src_race_acc, on_step=False, on_epoch=True, prog_bar=True)
        self.log("test/tgt_race_acc", self.test_tgt_race_acc, on_step=False, on_epoch=True, prog_bar=True)
        self.log("test/src_age_acc", self.test_src_age_acc, on_step=False, on_epoch=True, prog_bar=True)
        self.log("test/tgt_age_acc", self.test_tgt_age_acc, on_step=False, on_epoch=True, prog_bar=True)
    ...
    def configure_optimizers(self) -> tuple[list[Any], list[Any]]:
        ...
        optimizer_cls1 = self.hparams.optimizer_cls1(params=self.classifier_race.parameters())
        optimizer_cls2 = self.hparams.optimizer_cls2(params=self.classifier_age.parameters())
        ...
        #     }
        if self.is_MoG_prior:
            return [optimizer_vae_1, optimizer_vae_2, optimizer_score, optimizer_cls1, optimizer_cls2, optimizer_prior], []
        else:
            return [optimizer_vae_1, optimizer_vae_2, optimizer_score, optimizer_cls1, optimizer_cls2], []
